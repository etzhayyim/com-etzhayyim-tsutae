...
;; adapted for etzhayyim/com-etzhayyim-tsutae: this child repo has no manifest/
;; dir (that's a superproject-only convention) — schema.edn lives at repo root.
...
;; preserve-ns mode (tsutae addition, mirrors adr-key in the ADR generic
;; transform below): if a key already carries a meaningful namespace
;; (e.g. :cell/id, :actor/gates), keep it as-is — don't blindly re-prefix
;; with the file-derived ns-name, which would fragment cross-file attrs
;; (e.g. :cell/kind) into a different keyword per file. Only genuinely
;; bare keys (no namespace) get the file-derived ns-name prefix.
(defn preserve-or-namespaced-key [ns-name k]
...
(defn entity-from-map-preserve-ns
  "entity-from-map と同じだが、既に名前空間付きのキーはそのまま保持する
   （preserve-or-namespaced-key 使用）。idiomatic に既に :ns/key 形式の
   キーを持つファイル（cells/*.edn の :cell/*、manifest.edn の :actor/* 等）用。"
  [content ns-name]
  (into {:db/id -1}
        (map (fn [[k v]] [(preserve-or-namespaced-key ns-name k) (attr-value v)]))
        content))
...
(defn schema-attrs-preserve-ns
  ...
  (for [[k v] content]
    (let [{:keys [type card]} (classify v)]
      {:db/ident (preserve-or-namespaced-key ns-name k)
       :db/valueType type
       :db/cardinality card})))
...
(defn wrap-map-preserve! [rel-path ns-name]
  (let [f (io/file root rel-path)
    ...
    (if (already-tx-data? content)
      (println "skip (already tx-data):" rel-path)
      (let [entity (entity-from-map-preserve-ns content ns-name)
            attrs (schema-attrs-preserve-ns content ns-name)]
        (spit f (pr-str [entity]))
        (merge-schema! attrs)
        (println "wrapped (ns-preserving)" rel-path "->" (count entity) "attrs, fallback-ns=" ns-name)))))
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb edn-datomize.bb [wrap-map <path> <ns> | wrap-map-preserve <path> <fallback-ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
